...
public class Main_BJ_17471_게리맨더링_이현영 {
	...
	public static void powerset(int cnt) {
		if(cnt==N || total==Math.pow(2, N-1)) {
			...
			if(isConnectedA(isSelected) && isConnectedB(isSelected)){
				if(exista.size()+existb.size()==N) cal(isSelected);
			}
	...
	public static boolean isConnectedA(boolean[] A) {
		...
		for (int i = 0; i < N; i++) {
			//i번째가 false면
			if(!A[i]) {
				if(!exista.contains(i) && exista.size()==0) exista.add(i);
				for(int x: list[i]) {
					if(!A[x]) {
						if(exista.contains(x)) continue;
						exista.add(x);
						System.out.println("a노드 "+i+"가 인접한 "+x);
					}
				}
			}
		}
		System.out.println("지나간 노드 A "+Arrays.toString((exista.toArray())));
	...
	public static boolean isConnectedB(boolean[] B) {
		...
		for (int i = 0; i < N; i++) {
			//i번째가 true면
			if(B[i]) {
				if(!existb.contains(i) && existb.size()==0) existb.add(i);
				for(int x: list[i]) {
					if(B[x]) {
						if(existb.contains(x)) continue;
						existb.add(x);
						System.out.println("b노드 "+i+"가 인접한 "+x);
					}
				}
			}
		}
		System.out.println("지나간 노드 B "+Arrays.toString((existb.toArray())));
	...
	public static void cal(boolean[] AB) {
		...
		for (int i = 0; i < N; i++) {
			...
			if(!AB[i]) {
				suma+=nums[i];
				System.out.println("A 구역 "+i);
			}
				
			//true면 B구역
			else {
				sumb+=nums[i];
				System.out.println("B 구역 "+i);
			}
		}

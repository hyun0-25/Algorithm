...
public class 낚시왕 {
	...
	public static void main(String[] args) throws IOException {
		 ...
		 */
		for (int j = 1; j < R+1; j++) {
			System.out.println(Arrays.toString(isShark[j]));
		}
		System.out.println();
		for (int i = 0; i <= C; i++) {
//			isShark = new int[R+1][C+1];
	...
	public static void Fishing(int king) {
		...
		for (int i = 1; i <= R; i++) {
			...
			if(isShark[i][king]>=0) {
				for (int j = 0; j < sharks.size(); j++) {
					if(sharks.get(j).s==isShark[i][king]) sharks.remove(i);
				}
				score += isShark[i][king];
				isShark[i][king] = -1;
			}
		}
		//해당 열에 상어 없으면 0
	...
	public static void deleteShark(Shark shark) {
		...
		if(isShark[x][y]==-1) isShark[x][y]=size;
		...
		//그 위치의 상어와 크기 비교해서 
			...
			if(isShark[x][y]>size) sharks.remove(shark);
				...
				for (int i = 0; i < sharks.size(); i++) {
					if(sharks.get(i).s==isShark[x][y]) sharks.remove(i);
				}
				//큰 상어 넣기
				isShark[x][y]=size;
			}
